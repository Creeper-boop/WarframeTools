...
if len(last_used_arguments) > 1:
    overrides = last_used_arguments[1].replace("\"", "").replace(":", "=").replace("false", "0").replace("disabled", "0")\
        .replace("true", "1").replace("enabled", "1").replace("}", "").split(",")
    with open(steam_path + proton_compat_data + warframe_compat + "/DS.cfg", "r") as config:
        dedicated_server_config = config.readlines()

    try:
        launcher_settings = dedicated_server_config.index("[LauncherDedicatedServerSettings,LotusDedicatedServerSettings]\n")
    except ValueError:
